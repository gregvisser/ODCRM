 ...
 */
async function callSelfHostedLLM(companyName: string, website: string, scrapedData: {
  description: string
  html: string
  jsonLd: any[]
 ...
 */
function fallbackEnrichment(companyName: string, website: string, html: string, jsonLd: any[]): EnrichmentResult {
  const org = pickOrganization(jsonLd) || {}
  const description = extractMeta(html, 'description') || extractMeta(html, 'og:description') || ''
  const accreditations = extractAccreditations(html).join(', ')

  const foundingYear = org.foundingDate ? String(org.foundingDate).slice(0, 4) : ''
...
export async function enrichCompanyAbout(
  ...
  try {
    ...
    let result = await callSelfHostedLLM(companyName, normalizedWebsite, { description, html, jsonLd })

    // Fallback to scraping-only if LLM fails
    if (!result) {
      result = fallbackEnrichment(companyName, normalizedWebsite, html, jsonLd)
    }
    ...
    await prisma.customer.update({
      ...
      data: {
        ...
        // Update domain if website field doesn't exist yet
