...
async function main() {
  ...
  if (build.status !== 200) fail(`GET /api/_build returned ${build.status}, expected 200`)
  ...
  if (!sequenceId) {
    ...
    if (seqRes.status !== 200) fail(`GET /api/sequences returned ${seqRes.status}, expected 200`)
  ...
  if (createRes.status !== 201) {
    if (createRes.status >= 500) fail(`POST .../enrollments returned ${createRes.status} (must never 500)`)
    fail(`POST .../enrollments returned ${createRes.status}, expected 201. Body: ${JSON.stringify(createRes.data)}`)
  }
  ...
  if (pauseRes.status !== 200) {
    if (pauseRes.status >= 500) fail(`POST .../pause returned ${pauseRes.status} (must never 500)`)
    fail(`POST .../pause returned ${pauseRes.status}, expected 200. Body: ${JSON.stringify(pauseRes.data)}`)
  }
  ...
  if (resumeRes.status !== 200) {
    if (resumeRes.status >= 500) fail(`POST .../resume returned ${resumeRes.status} (must never 500)`)
    fail(`POST .../resume returned ${resumeRes.status}, expected 200. Body: ${JSON.stringify(resumeRes.data)}`)
  }
  ...
  if (listRes.status !== 200) {
    if (listRes.status >= 500) fail(`GET /api/enrollments returned ${listRes.status} (must never 500)`)
    fail(`GET /api/enrollments returned ${listRes.status}, expected 200`)
  }
  ...
  if (notFoundRes.status !== 404) {
    if (notFoundRes.status >= 500) fail(`GET .../nonexistent returned ${notFoundRes.status} (must never 500)`)
    fail(`GET .../nonexistent expected 404, got ${notFoundRes.status}`)
  }
