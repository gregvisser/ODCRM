...
| Exactly one source of active client; API carries x-customer-id; no silent fallbacks (e.g. prod-customer-1). | Single source: localStorage `currentCustomerId`; api.ts sets X-Customer-Id; fallback `prod-customer-1` when empty (`src/utils/api.ts` L5–9, L49, L75–76). | Silent fallback to prod-customer-1; no explicit empty-state when no client selected. | 0 | Medium | PR2: Remove silent tenant fallback(s) + add explicit empty-state UX when no active client. |
