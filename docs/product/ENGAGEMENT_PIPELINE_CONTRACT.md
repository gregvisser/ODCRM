...
- **Stage 2D (shipped):** Scheduled send-queue metrics monitor — GitHub Action runs every 6 hours (and on `workflow_dispatch`). Uses `docs/ops/send-queue-metrics-customers.txt` as customer list, runs `npm run send-queue:metrics:report` with `ODCRM_FAIL_ON=failed,stuckLocked`, uploads `send-queue-metrics-report.json` as artifact. Secret `ODCRM_ADMIN_SECRET` from GitHub Secrets only; never logged. Empty customer list fails with clear message. Self-test: `npm run test:send-queue-metrics-workflow-stage2d`. No runtime app changes.
