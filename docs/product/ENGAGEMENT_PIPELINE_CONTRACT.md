...
- **Stage 1E (shipped):** Admin dry-run tick endpoint `POST /api/send-queue/tick`: requires `X-Admin-Secret`, body `{ customerId, limit?, dryRun? }` (dryRun must be true). Runs one pass: selects due QUEUED items for that customer, locks them, then requeues each with the same dry-run logic (QUEUED + lastError). Idempotent-safe; no sending. Use for operational validation and to unstick queues without enabling send.
- **Stage 1F (shipped):** Canary-only live sending (step 0 only). Live send only when all are true: `ENABLE_SEND_QUEUE_SENDING === "true"`, `ENABLE_LIVE_SENDING === "true"`, `SEND_CANARY_CUSTOMER_ID` set and equals item `customerId`; optional `SEND_CANARY_IDENTITY_ID` (if set, chosen identity must match). Tick accepts `dryRun=false` only when `ODCRM_ALLOW_LIVE_TICK === "true"` and canary gates pass. Per-tick/per-iteration cap: `LIVE_SEND_CAP` (default 10). On send success: status=SENT, sentAt set, lock cleared; on send failure: status=QUEUED, lastError=`SEND_FAILED: <msg>` (truncate 500), never leave LOCKED. Audit events for each send attempt (success/failure). Self-test: `npm run test:send-queue-stage1f` (default preflight only); live with `ODCRM_TEST_LIVE=1` and canary env.
- **Stage 1G (shipped):** Tick supports `ignoreWindow=true` (body param) for canary-only, admin-only bypass of send-window check. Requires `ODCRM_ALLOW_LIVE_TICK_IGNORE_WINDOW === "true"` in addition to live tick + canary gates. Worker behavior unchanged (never uses bypass). One log line per bypass send; `EnrollmentAuditEvent` with eventType `send_window_bypass` for proof. Self-test: `npm run test:send-queue-stage1g` (default preflight; live with `ODCRM_TEST_LIVE=1` and canary env). Ops-only switch; turn off after verification.
- **Stage 2:** Queue worker + observability (send queue consumer, metrics, alerts). Future PR.
- **Stage 2A (shipped):** Send Queue metrics endpoint — `GET /api/send-queue/metrics?customerId=<cust_...>`. Auth: `X-Admin-Secret` (same as tick). Returns operational metrics for one customer: `countsByStatus`, `dueNow`, `oldestScheduledFor` / `newestScheduledFor`, `lockedCount`, `stuckLocked` (LOCKED older than 15 min), `lastSentAt`, `lastErrorSample`. No behavior change to sending or tenant logic. Self-test: `npm run test:send-queue-metrics-stage2a` (requires `ODCRM_TEST_ADMIN_SECRET`, `ODCRM_TEST_CUSTOMER_ID`).
...
- **Stage 2B (shipped):** Ops metrics report script — `npm run send-queue:metrics:report`. Pulls `GET /api/send-queue/metrics` for multiple customers (env: `ODCRM_ADMIN_SECRET`, `ODCRM_CUSTOMER_IDS` comma-separated, `ODCRM_METRICS_API_BASE` optional, `ODCRM_OUTPUT=json|pretty`). Prints table-like report or JSON. **Admin-only:** admin secret must never be used in browser/frontend; run only from secure ops environment. Self-test: `npm run test:send-queue-metrics-stage2b` (requires `ODCRM_TEST_ADMIN_SECRET`, `ODCRM_TEST_CUSTOMER_IDS`). No sending logic or tenant isolation changes.
- **Stage 2C (shipped):** Metrics report enhancements — customer IDs from file (`ODCRM_CUSTOMER_IDS_FILE`, newline/comma/whitespace, `#` comments), write JSON to file (`ODCRM_OUTPUT_FILE`), optional sort (`ODCRM_SORT`: dueNow, queued, failed, stuckLocked, lastSentAt), optional fail-on thresholds (`ODCRM_FAIL_ON`: failed, stuckLocked, dueNow, queued — exit 2 if any > 0 for CI). Self-test: `npm run test:send-queue-metrics-stage2c` (requires `ODCRM_TEST_ADMIN_SECRET`, `ODCRM_TEST_CUSTOMER_IDS` or `ODCRM_TEST_CUSTOMER_IDS_FILE`). Admin-only; no backend/frontend changes.
- **Stage 2D (shipped):** Scheduled send-queue metrics monitor — GitHub Action runs every 6 hours (and on `workflow_dispatch`). Uses `docs/ops/send-queue-metrics-customers.txt` as customer list, runs `npm run send-queue:metrics:report` with `ODCRM_FAIL_ON=failed>0,stuckLocked>0`, uploads `send-queue-metrics-report.json` as artifact. Secret `ODCRM_ADMIN_SECRET` from GitHub Secrets only; never logged. Empty customer list fails with clear message. Self-test: `npm run test:send-queue-metrics-workflow-stage2d`. No runtime app changes.
- **Stage 2E (shipped):** ODCRM_FAIL_ON extended with numeric thresholds. Rules: token only (e.g. `failed` => failed>0) or comparator: `queued>=10`, `dueNow>5`, `stuckLocked=0`. Allowed metrics: failed, stuckLocked, dueNow, queued, sent, locked, skipped. Any rule triggers => exit 2. Self-test: `npm run test:send-queue-metrics-stage2e` (unit test, no network). No backend/frontend changes.
- **Stage 3A (shipped):** Send-queue preview — `GET /api/send-queue/preview`. Tenant-scoped (X-Customer-Id required); read-only, no sending, no DB mutations. Returns per-item action (WAIT/SKIP/SEND) and reasons (not_due_yet, already_sent, locked, missing_identity, missing_recipient_email, unknown). Optional `?enrollmentId=&limit=` (default 20, max 100). See [SEND_QUEUE_PREVIEW_STAGE3A](./SEND_QUEUE_PREVIEW_STAGE3A.md). Self-test: `npm run test:send-queue-preview-stage3a` (guardrail: 400 without tenant or 401/403; 200 with X-Customer-Id in local/dev).
