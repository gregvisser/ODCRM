...
function loadMarketingLeadsFromStorage(): Lead[] {
...
function AccountsTab({ focusAccountName }: { focusAccountName?: string }) {
  ...
  const [dashboardLeads, setDashboardLeads] = useState<Lead[]>(() => loadMarketingLeadsFromStorage())

  useEffect(() => {
    const off = on('leadsUpdated', () => {
      setDashboardLeads(loadMarketingLeadsFromStorage())
    })
    return () => off()
  }, [])

  const refreshDashboardLeads = async () => {
    try {
      ...
      // Keep the legacy leads store in sync too (other screens read it).
      setJson(OdcrmStorageKeys.leads, allLeads)
      setItem(OdcrmStorageKeys.leadsLastRefresh, new Date().toISOString())

      setDashboardLeads(allLeads)
      emit('leadsUpdated')
      toast({ title: 'Leads refreshed', description: `Loaded ${allLeads.length} leads.`, status: 'success', duration: 2500, isClosable: true })
    } catch (e: any) {
      toast({ title: 'Refresh failed', description: e?.message || 'Unable to refresh leads.', status: 'error', duration: 3500, isClosable: true })
    }
  }

  // Compute the SAME “Unified Lead Performance” metrics as MarketingLeadsTab.
  const unifiedAnalytics = useMemo(() => {
    ...
    // Daily target matches MarketingLeadsTab (weekly/7 or monthly/daysInMonth).
    ...
    const leadsWithDates = dashboardLeads
      .map((lead) => {
        const dateValue =
          lead['Date'] ||
          lead['date'] ||
  ...
  }, [accountsData, dashboardLeads])

  return (
    <>
      {/* Unified Lead Performance (copied from MarketingLeadsTab for 1:1 parity) */}
      <Box
        mb={6}
        bg="bg.surface"
        borderRadius="lg"
